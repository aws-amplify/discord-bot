...
async function getDiscordUserId(ghUserId: string) {
  const discordAccount = await prisma.user.findFirst({
    where: {
      accounts: {
        some: {
          provider: 'github',
...
if (import.meta.vitest) {
  ...
  describe('Successful recieving webhook and applying roles', () => {
    test('Getting discord user id', async () => {
      await expect(getDiscordUserId(
        String(addedPayload.body.membership.user.id)
      )).rejects.toThrowError(`Discord account not found for GitHub user ${addedPayload.body.membership.user.id}`)
    })
  })

  describe('Failure recieving webhook and applying roles', () => {
    test('Getting discord user id, user not in database', async () => {
      await expect(getDiscordUserId(
        String(addedPayload.body.membership.user.id)
      )).rejects.toThrowError(`Discord account not found for GitHub user ${addedPayload.body.membership.user.id}`)
    })

    test('Adding roles, user not in the database', async () => {
      const response = await post({ request: createRequest(removedPayload) })
