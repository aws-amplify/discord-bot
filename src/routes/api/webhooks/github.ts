...
function verifyGithubWebhookEvent(req) {
  const token = process.env.GITHUB_WEBHOOK_SECRET
  const sig = Buffer.from(req.get(sigHeaderName) || '', 'utf8')
  const hmac = crypto.createHmac(sigHashAlg, token)
...
export async function post({ request }) {
  ...
  // verify webhook event, is this event trustworthy? using GITHUB_WEBHOOK_SECRET
