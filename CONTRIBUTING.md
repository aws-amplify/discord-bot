...
> **Note**
> dotenv files are loaded using Vite's `loadEnv` and `local` dotenv files are not supported when creating secrets. Additionally, we must be sure to pass a valid environment name such as `main` or `next`
...
> **Note**
> You do not need to set up a GitHub organization to test with locally if you do not plan to work on GitHub-related features
...
> **Note**
> in-source testing with Vitest does not tree-shake dependencies, which could lead to ambiguous errors caused by circular references
